function [S,P] = RichClubHuman(Adj,averageCoexpression,analyzeWhat,measure,coexpMeasure,whatMask,whatAdj,networkType,useOnlyInterneurons,doRanksum)
% ------------------------------------------------------------------------------
...
% ------------------------------------------------------------------------------
[~,~,nodeData] = AdjLabelNodes(labelNodesHow,Adj,extraParam,networkType);
...
for j = 1:length(whatLinks)
    ...
    switch measure
        ...
        case 'median'
            realTrajectory = cellfun(@nanmedian,allHubHub{1}(:,j));
    end
